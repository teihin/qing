...
import type { GameRewardPoolState } from "../types";
...
export default function RewardPoolsPage({ can, notify }: {
  ...
  const load = useCallback(async (showState = false) => {
    ...
    try {
      applyState(await api<GameRewardPoolState>("/api/configuration/reward-pools"));
      if (showState) notify("已重新读取游戏服务中的奖池金额");
    } catch (reason) {
  ...
  const refresh = () => {
    if (changedItems.length > 0 && !window.confirm("当前有未保存的奖池修改，确定放弃并重新读取吗？")) return;
    void load(true);
  };
  ...
  return (
    <div className="page-stack">
      <PageHeader eyebrow="REWARD POOL CONTROL" title="奖池设置" description="页面统一按元输入和展示，保存时由系统自动换算为分。" actions={
        <span className="configuration-status is-live"><i />游戏服务实时数据</span>
      } />

      {!data ? <section className="panel"><LoadingBlock label="正在读取各皮池奖池" /></section> : <>
