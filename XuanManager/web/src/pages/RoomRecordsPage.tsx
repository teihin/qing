...
function RoomRecordList({ items, onOpen }: { items: RoomRecordListItem[]; onOpen: (roomId: string) => void }) {
  return <div className="table-wrap"><table className="room-record-list-table"><thead><tr><th>房间</th><th>地九王</th><th>参与玩家</th><th>局数 / 人数</th><th>累计带入</th><th>房间时间</th><th>状态</th><th>操作</th></tr></thead><tbody>{items.map((item) => <tr key={item.roomId} className="room-record-list-row" tabIndex={0} onClick={() => onOpen(item.roomId)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(item.roomId); } }} aria-label={`查看房间 ${item.roomId} 战绩`}><td><strong className="room-list-id">{item.roomId}</strong><small className="cell-subtitle">{item.roomName || "未记录房间名"}</small></td><td>{item.isDijiuKing ? <DijiuKingBadge /> : null}</td><td><span className="room-list-participants">{item.participants.slice(0, 3).join("、") || "未记录"}</span>{item.participants.length > 3 && <small className="cell-subtitle">另有 {item.participants.length - 3} 名玩家</small>}</td><td><strong>{item.roundCount} 局</strong><small className="cell-subtitle">{item.playerCount} 名玩家</small></td><td><strong>{scoreFormatter.format(item.totalBuyIn)}</strong></td><td><span>{item.startedAt || item.recordedAt || "—"}</span><small className="cell-subtitle">至 {item.endedAt || item.recordedAt || "—"}</small></td><td><span className="room-list-status"><i />已结算</span></td><td><button type="button" onClick={(event) => { event.stopPropagation(); onOpen(item.roomId); }}>查看战绩</button></td></tr>)}</tbody></table></div>;
}
...
function PlayerStandings({ data }: { data: RoomRecordResponse }) {
  return <section className="panel"><div className="toolbar player-toolbar"><div><strong>玩家总战绩排名</strong><span>优先按实际结算返还减累计带入计算最终输赢，并标记原战绩表异常</span></div><span className="toolbar__count">{data.players.length} 名玩家</span></div><div className="table-wrap"><table className="room-player-table"><thead><tr><th>排名</th><th>玩家</th><th>座位</th><th>参与局数</th><th>累计带入 / 返还</th><th>最终输赢</th><th>进入 / 离开时间</th></tr></thead><tbody>{data.players.map((player, index) => <tr key={player.id}><td><span className={`room-rank room-rank--${index + 1}`}>{index + 1}</span></td><td><strong>{player.playerName || "未设置昵称"}</strong><small className="cell-subtitle">ID {player.playerId}</small></td><td>{player.seat + 1} 号位</td><td>{player.roundsPlayed} 局</td><td><strong>{scoreFormatter.format(player.totalBuyIn)} / {player.scoreSource === "settlement" ? scoreFormatter.format(player.settlementReturn) : "未记录"}</strong><small className="cell-subtitle">带入 / 结算返还</small></td><td><strong className={`room-score room-score--${player.result}`}>{formatScore(player.score)}</strong>{player.scoreMismatch && <small className="room-score-warning">原战绩表 {formatScore(player.recordedScore)}</small>}</td><td><span>{player.joinedAt || "—"}</span><small className="cell-subtitle">至 {player.leftAt || "—"}</small></td></tr>)}</tbody></table></div></section>;
}

function RoundRow({ round, onOpen }: { round: RoomRecordRound; onOpen: () => void }) {
  return <article className="room-round-row"><div className="room-round-number"><span>ROUND</span><strong>{round.round}</strong></div><div className="room-round-time"><strong>第 {round.round} 局</strong><span>{round.playedAt}</span></div><div className="room-round-stat room-round-stat--win"><span>本局赢分</span><strong>+{scoreFormatter.format(round.totalWin)}</strong></div><div className="room-round-stat room-round-stat--loss"><span>本局输分</span><strong>−{scoreFormatter.format(round.totalLoss)}</strong></div><div className="room-round-stat"><span>参与 / 操作</span><strong>{round.playerCount} 人 · {round.actionCount} 次</strong></div><div className="room-round-net"><span>差额</span><strong className={`room-score room-score--${round.netScore > 0 ? "win" : round.netScore < 0 ? "loss" : "draw"}`}>{formatScore(round.netScore)}</strong></div><button type="button" onClick={onOpen}>查看牌面与过程</button></article>;
}
