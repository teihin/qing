...
if [[ -r "${pid_file}" ]]; then
  ...
  if [[ -n "${running_pid}" ]] && kill -0 "${running_pid}" 2>/dev/null; then
    ...
    echo "XuanManager is already running"
    exit 0
  fi
fi
...
for _ in {1..40}; do
  if curl --fail --silent --max-time 1 "http://127.0.0.1:8891/api/health" >/dev/null; then
    "${deploy_dir}/install-caddy-route.sh"
    echo "XuanManager started"
    exit 0
  fi
  sleep 0.25
done

echo "XuanManager did not become healthy" >&2
exit 1
