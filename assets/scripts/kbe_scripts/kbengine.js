...
/*-----------------------------------------------------------------------------------------
												global
-----------------------------------------------------------------------------------------*/
KBEngine.PACKET_MAX_SIZE		 = 150000;
KBEngine.PACKET_MAX_SIZE_TCP	 = 146000;
KBEngine.PACKET_MAX_SIZE_UDP	 = 147200;
...
KBEngine.Entity = KBEngine.Class.extend(
{
	...
	set_gold(old)
	{
		KBEngine.Event.fire("set_gold",this.old);
	},
	set_gold2(old)
	{
		KBEngine.Event.fire("set_gold2",this.old);
	},
	set_name(old)
	{
		KBEngine.Event.fire("set_name",this.name);
	},
	set_level(old)
	{
		KBEngine.Event.fire("set_level",this.level);
	},
	set_photo(old)
	{
		KBEngine.Event.fire("set_photo");
	},
	set_stone(old)
	{		
		KBEngine.Event.fire("set_stone", this.old);
	},
	set_hongli(old)
	{            
		KBEngine.Event.fire("set_hongli");
	},
	set_playcount_status(old)
	{
		KBEngine.Event.fire("set_playcount_status");
	},
	set_playcount_count(old)
	{
		KBEngine.Event.fire("set_playcount_count");
	},
	set_super_percent(old)
	{
		KBEngine.Event.fire("set_super_percent");
	},
	set_bigagentid(old)
	{
		KBEngine.Event.fire("set_bigagentid");
	},
	set_bigagentid2(old)
	{
		KBEngine.Event.fire("set_bigagentid2");
	},
	set_supperAgentID(old)
	{
		KBEngine.Event.fire("set_supperAgentID");
	},
	set_client_status(old)
	{
	...
	set_club_gold(old)
	{
		KBEngine.Event.fire("set_club_gold", this.club_gold);
	},	
	set_club_join(old)
	{
		KBEngine.Event.fire("set_club_join", this.club_join);
	},
	set_club_make(old)
	{
		KBEngine.Event.fire("set_club_make", this.club_make);
	},
	set_club_gold2(old)
	{
		KBEngine.Event.fire("set_clubgold2", this.club_gold2);
	},
	set_club_join2(old)
	{
		KBEngine.Event.fire("set_club_join2", this.club_join2);
	},
	set_club_make2(old)
	{
		KBEngine.Event.fire("set_club_make2", this.club_make2);
	},
	set_club_manage(old)
	{
		KBEngine.Event.fire("set_club_manage", this.club_manage);
	},
	set_chiefAgentID(old)
	{
		KBEngine.Event.fire("set_chiefAgentID");
	},
	set_role(old)
	{
		KBEngine.Event.fire("set_role");
	},
	set_jiangli(old) //总裁未提取
	{
		KBEngine.Event.fire("set_jiangli");
	},


	set_all_hongli(old)
	{
		KBEngine.Event.fire("set_all_hongli");
	},
	set_use_hongli(old)
	{
		KBEngine.Event.fire("set_use_hongli");
	},
	set_fenhong(old)
	{
		KBEngine.Event.fire("set_fenhong");
	},
	set_use_fenhong(old)
	{
		KBEngine.Event.fire("set_use_fenhong");
	},
	set_use_all_fenhong(old)
	{
		KBEngine.Event.fire("set_use_all_fenhong");
	},
	set_big_percent(old)
	{
		KBEngine.Event.fire("set_big_percent");
	},
	//奖池收益
	set_hongli2(old)
	{            
		KBEngine.Event.fire("set_hongli2");
	},
	set_all_hongli2(old)
	{
		KBEngine.Event.fire("set_all_hongli2");
	},
	set_use_hongli2(old)
	{
		KBEngine.Event.fire("set_use_hongli2");
	},
	set_client_prop(old)
	{
		KBEngine.Event.fire("set_client_prop");
	},
	set_paper(old)
	{
		KBEngine.Event.fire("set_paper",this.old);
	},
...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	//自定义的baseapp地址
	this.defBaseAppIP = this.args.defBaseAppIP;
	...
	this.reset = function()
	{
		...
		KBEngine.app.clientVersion = "1.3.12";//"1.2.7";
	...
	this.send = function(msg)
	{
		...
		{
			KBEngine.INFO_MSG('KBEngine::socket error()');
		}
	}
	...
	*/
	...
	{  
		if(noconnect)
		{
			...
			var serverAddr = this.getServerAddr(KBEngine.app.defBaseAppIP, KBEngine.app.baseappPort);
	...
	{
		KBEngine.ERROR_MSG("KBEngineApp::Client_onLoginBaseappFailed111: failedcode(" + KBEngine.app.serverErrs[failedcode].name + ")!");
