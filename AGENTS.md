...
- 2026-07-23 用户决定把通用键盘定位延后处理，当前先采用临时可见性方案：在恢复后的Cocos原生`setTopMargin()`计算上额外向上移动一个EditText自身高度，即从`topMargin - height`改为`topMargin - 2 * height`，不使用固定像素；GL画布不自动`complete()`的修复继续保留。完整Debug构建已通过，待重新安装确认输入栏完整可见。
