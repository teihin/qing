...
- 新后台后端应部署在游戏服务器本机；XuanManager 自有 `mgr_` 表统一存放在本机 MySQL 的 `webcm` 数据库。游戏业务数据默认从 `kbedm` 只读查询；只有注册写入 `third_marketing_info`、长期公告写入 `usr_hash_info` 等用户明确批准的流程可以使用专门授予的最小写权限。内部管理接口固定为 `http://127.0.0.1:8890`。浏览器前端不得直连数据库或透传旧管理接口，数据库和内部 HTTP 均不得为开发方便开放到公网。
...
- 2026-08-09 已新增供游戏客户端注册页调用的公开接口 `POST /api/game/registrations`，规范请求字段为 `invitationCode`、`nickname`、`loginName`、`password`、`avatarIndex`，并兼容接口文档旧字段 `upper_guuid`、`player_wxid`、`player_wxname` 及头像别名 `photo`。邀请码必须为6位数字并命中代理/盟主候选账号；昵称为1～32位且只能使用中文或英文字母；登录账号必须为6位英文字母或数字；头像编号必须为未补零的字符串1～20，旧客户端不传时默认1。登录账号同时检查 `third_marketing_info.player_wxid`、`tbl_Account.sm_wxID` 和 `kbe_accountinfos.accountName`，最终仍由 `player_wxid` 唯一索引处理并发冲突。注册主记录只写 `kbedm.third_marketing_info`；旧游戏要求的 MD5 密码兼容逻辑保留，但明文和摘要均不写日志、审计或响应。公开接口允许无凭据跨域 POST，按可信代理后的来源IP限制10分钟10次；浏览器走纯HTTP时密码仍会明文传输，属于用户已接受但应持续保留的风险。
...
- 2026-08-10 注册头像选择已按用户反馈改为与游戏内相同的列表式流程：注册表单只显示当前头像和“点击选择头像”，原左右切换按钮在 Prefab 中停用；点击头像打开独立二级弹窗，弹窗内固定展示5列×4行共20张本地头像、当前项勾选标记、标题、副标题、关闭按钮和遮罩，点击任一头像立即更新注册选择并关闭二级弹窗。入口、二级弹窗和20个头像按钮全部是 `assets/resources/UI/panelLogin.prefab` 内的真实序列化节点，可直接在Creator中编辑预览；`panelLogin.ts` 只处理开关、选中态和数值提交，不运行时创建或排版UI。服务端 `avatarIndex` 1～20及首次登录补写逻辑不变。Prefab共450个序列化对象，20个头像按钮/20个唯一SpriteFrame、父子层级和引用校验通过；干净临时项目的Cocos Creator 2.4.13 Web Mobile Debug构建及编译产物字符串检查通过，离线按真实坐标合成确认弹窗无重叠越界。当前7456仍是修改前已运行的旧内存预览，独立Web包又被项目已有的`GameDef`重复注册错误阻断，因此最终运行态点击仍需在Creator重新启动预览后确认。
- 2026-08-10 防盗号设备绑定的确认范围仅要求“同一手机卸载 App 后重新安装，设备 ID 保持不变”，不要求恢复出厂、刷机或换机后仍识别为原设备。待实现方案：Android 使用同一签名和系统用户下的 `ANDROID_ID`；iOS 首次生成随机 UUID 并保存到关闭同步的 `ThisDeviceOnly` Keychain 项；客户端通过原生桥取得标识，服务端只保存带服务端密钥的 HMAC 结果，不保存原始标识。恢复出厂、刷机、系统用户变化、签名变化或 Keychain 被清除均按新设备处理。纯网页无法读取等价的稳定硬件 ID：普通网页第一版只能生成浏览器 UUID，冗余保存到 IndexedDB、localStorage 和持久 Cookie，并申请持久存储，清除站点数据、无痕模式或更换浏览器后仍会变化；若网页也必须在清缓存或重装 PWA 后保持绑定，应使用 HTTPS 下的 WebAuthn 平台凭证，并由服务端拒绝可备份/可同步凭证，只接受 `BE=0` 的单设备凭证，不支持时不能宣称严格防盗号。上述方案均尚未编码和真机/多浏览器验证。
