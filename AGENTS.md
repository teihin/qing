...
2026-07-22 已将 `/Applications/Cocos/Creator/2.2.1` 与 Cocos 官方 `2.2.1-release` 源码基线逐文件比较，并映射到 2.4.13。当前结论尚待用户确认，未对 2.4.13 执行合并。

- JavaScript 引擎层没有项目定制逻辑。EditBox、事件、资源库、实例化、粒子等差异均为官方发行补丁、BOM 或版本元数据，不应迁移。
- `cocos/platform/CCImage.cpp/.h` 有明确的 Native 加密 PNG 兼容补丁：识别自定义文件头，重建 PNG 头尾，并用源码内固定 key 循环 XOR 还原中间数据。它只影响重新编译的 Android/iOS/Windows Native 包，不影响 Web、编辑器导入或未重编译的内置 Simulator。
- 当前项目 `assets` 下 1278 个 PNG 均为标准 PNG；工作区和 `runtime-src.zip` 未发现加密图片、配套加密器或加密脚本。因此图片补丁是否仍属当前发布流程待确认，验证时需要旧 APK/IPA、热更资源包、加密工具或至少一个加密样本。
- 若迁移图片补丁，只在 2.4.13 的 `CCImage` 解码入口做最小适配，不能用 2.2.1 整文件覆盖，以免丢失 2.4.13 的 ASTC 等新增能力；同时要补长度、分配和解码失败保护。
...
- 暂定主风格为黑金、高档、克制的东方帝王风；第一阶段从竖屏登录界面开始，先提供整页效果图供用户确认，再进行切图和原资源替换。
- 第一版登录页概念采用秦代宫阙、黑曜石、暗金纹样与鎏金品牌字，保留账号、密码、登录按钮的原交互层级。该方案尚待用户确认；概念图中的品牌字形在正式制作时需重绘为准确、可控的“秦”字矢量标志。
- 用户反馈第一版缺乏高级感和科技感；第二版方向调整为“未来东方奢华”，减少传统宫殿与厚重纹样，采用黑曜石玻璃、钛金切边、全息秦印扫描环、细线电路光轨和轻薄科技面板。第二版仍为待确认概念稿。
