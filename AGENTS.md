...
- 当前 Android 工具链使用 AGP/Gradle 8.0.2、NDK 25.1、compile/target SDK 28 和 Build Tools 30.0.3。AGP 8.0.2 自带的旧 `aapt2` 无法解析本机 Android 35 `android.jar`，会报资源表偏移重叠；如需满足新商店 target SDK 要求，应整体升级 AGP、Gradle 和 Cocos 原生适配后再升 SDK，不能只把 `PROP_COMPILE_SDK_VERSION` 改为 35。
- 本次 Debug APK 已验证 ZIP 完整，且同时包含 `arm64-v8a` 与 `armeabi-v7a` 的 `libcocos2djs.so`；尚未做安装、启动、登录和真机功能回归。
...
- 本轮按用户要求不迁移 GCloudVoice、TalkingData 和微信授权登录：当前 Android 工程没有对应 JAR/AAR/SO、Java 类、Manifest Activity、录音权限或 SDK 初始化；`MobileManager.ts`保留旧公开方法名兼容现有调用，但微信、语音和统计入口均为空操作，不再执行 Native 反射或微信 Token 请求。历史 PictureSelector/相册选头像也未迁移，因为当前头像流程已经改成本地20张资源选择。
...
- 2026-07-23 复核当前生成工程：2.4.13 的 `config.hpp` 确实把 V8 Inspector 能力编译进引擎，`Classes/AppDelegate.cpp` 也保留 `jsb_enable_debugger("0.0.0.0", 6086, false)`，但该调用受 `COCOS2D_DEBUG > 0` 条件保护。因此 Debug 原生包会开放 6086 端口，当前 Release/Archive 包不会开放；`build/jsb-link/project.json` 的 `debugMode: 1` 只控制 JS 日志显示级别，不会绕过该编译条件。若为测试 Release 临时开放，应使用单独宏并在正式发布前关闭，避免局域网暴露调试器及脚本执行能力。
...
- 2026-07-23 已确认本游戏后续热更新包通过 SFTP 上传到 `154.37.155.17:2233`，账号为 `client_update`，本机使用 `~/.ssh/id_ed25519_newserver`；该账号被限制为仅 SFTP，不能使用 SSH Shell。SFTP 登录后的受限根目录显示为 `/`，上传更新内容时进入 `/html`：`latest.json` 放 `/html/latest.json`，更新压缩包放 `/html/packages/<包名>.zip`。网页对应 `http://154.37.155.17/latest.json` 与 `http://154.37.155.17/packages/<包名>.zip`。上传前仍需核对本次实际清单文件名、压缩包名和版本，不能把示例 `xxx.zip` 当成固定名称。
...
- 当前实际生成的`build/jsb-link`原生工程没有`Battle`文本：Android `app_name`为`qing`，iOS显示名使用当前`PRODUCT_NAME`；Android/iOS源图目录共24张图标均为Cocos默认图标而非旧`BATTLE`图标。它们不会暴露上一项目标题，但也尚未完成“秦”品牌应用图标设计，正式发布前仍需整套替换。
...
- 2026-07-23 已生成首轮应用图标四方向提案并保存为`art_sources/app_icon/qin_app_icon_concepts_v1.png`：A“帝王金印”强调小尺寸品牌辨识，B“玄甲战旗”强调战斗冲击，C“牌九王印”强调玩法属性，D“咸阳宫门”强调秦代场景氛围。四版均沿用已确认的定制金属“秦”字、黑曜石与暗金主色和少量朱砂点缀；当前仅为选择稿，尚未裁切或替换Android/iOS正式图标，最终方向待用户确认。
