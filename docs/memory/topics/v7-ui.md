...
2026-09-13 用户确认同页三个主 Tab，只有手数榜有 1/2/5/10/20皮；24–26 v3 已原路径覆盖（941×1672），其他 53 张不变。09-14 正式页已组件化并修复金色选中态；此前分页版本的 Creator、长短屏验证见[实施交接](../handoffs/2026-09-14-leaderboard-v8.md)。**最新要求改为三榜无限滚动、近底预取并移除分页**，正式 Prefab/脚本已保存，15 项模拟回归、五档静态/类型检查通过；本次 Mac 锁定，Creator 导入与网页滚动待验，不能沿用前阶段预览证明本次功能。详见[无限滚动交接](../handoffs/2026-09-14-leaderboard-infinite.md)；真实排行/领奖、构建和真机仍待验。
...
- 我的资料卡保留真实头像、昵称、ID、金币和五项统计；五档底皮手数（1/2/5/10/20 皮）由 `panelMain.set_playcount_count()` 从 `Account.playcount_count` 读取：服务端按底皮档位升序以“#”分隔下发（0.1/0.3、0.2/0.5、1/3、2/5、5/10、10/20、20/40、50/100），1~20 皮对应下标 2~6，字段缺失才显示占位“—”，不能回退静态 0。复制 ID 使用独立透明 Button 和真实返回结果。Prefab 默认非代理五入口，真实代理账号恢复六入口，不能为统一静态截图删除该差异。来源：第 57 行。
...
- 2026-09-13 战绩改为大厅共用 `lobby_scene_long_v8.png`，替代专用场景和拼接延展；定稿盾牌独立透明裁切，日期/选中底板、列表底框及行框九宫格，原图数字制作成动态 LabelAtlas。旧场景与六张旧日期状态图在引用检查后移到 `art_sources/v8-repairs/record/retired/`，正式 PNG 净减少 1,210,646 字节。Creator/静态四档/网页短长屏和日期状态已查；真实记录、分页和回放仍待真实数据，见[战绩修复](../handoffs/2026-09-13-record-v8-repair.md)。
...
- 2026-09-05 的无人物主视觉为历史 V7 实现，已由 V8-new 定稿替代。2026-09-13 修正 V8 记录类型的左锚点与居中位置不配套问题，原底板 7 像素窄带拉伸竖纹改用内置图像编辑清理；正式 Prefab、干净源图与提取入口同步，Creator 已导入。网页刷新后处于登录页，修复后的真实记录显示待重新登录核对；见[列表修复](../handoffs/2026-09-13-gift-list-alignment.md)。
...
- 2026-09-07 `design-previews/2026-09-07-V7桌内统一风格重设计-v6/` 替代旧偏黄黑金、圆桌背景和过大侧栏方向。其中小型两列牌局菜单已经用户确认并正式写入 `assets/resources/UI/panelGameView.prefab`、`assets/Scenes/drh8.fire`：面板与八个菜单入口采用确认稿直切资源；业务节点、事件和权限显隐保留，`cc.Layout` 以两列网格自动忽略非激活子项，因此隐藏入口不会留下空格。首轮从概念图缩取的快捷图标因内图形偏小/错位被否决，六个桌边快捷图标已改为统一 128×128 冷蓝金三环圆盘，图形统一放大居中，“菜单”和“记录”保持不同语义。首版纯文字空位圆环曾因缺乏设计感被否决，后曾使用座椅、加号和“空位”铭牌徽标；该空位视觉已被 2026-09-14 用户最新竞品截图要求替代：正式八座位现用无字单圈蓝灰座椅，顶部奖池改为浅金分格数字，两份 Prefab/Scene 已导入，长短屏和金额边界通过，真实交互待验，见[本次交接](../handoffs/2026-09-14-room-seat-jackpot.md)。其余全高半屏实时战绩、全高半屏牌局回顾、顶部奖池弹窗和玩家信息弹窗仍是待确认/未实施概念；实时战绩不显示头像，回顾使用紧凑玩家行和放大牌面，桌面背景为当前全桌面而非圆桌。
