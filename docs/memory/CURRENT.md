...
- 房间邀请后续按用户新确认的独立蓝金稿实施，替代该弹窗原版通用样式；动态数据与原交互保留。正式Prefab/9张资源已核对Creator导入，业务mock通过，真实邀请回包及真机待验。见[房间邀请交接](handoffs/2026-09-20-room-invite-approved.md)。 后续按实机反馈隐藏下方重复文案；真实Cocos组件可读回底皮/人数，用户旧页面与本次Prefab重新导入仍待复验。

- 带入积分内“金币余额不足…请先充值”子弹窗同样统一为温馨提示风格，保留动态金额、取消/充值和右上关闭；Prefab/Scene与静态叠层已核对，实际点击待验，见[交接](handoffs/2026-09-20-insufficient-dialog-style.md)。
...
- `panelGivePad` 已按确认稿换肤，保留动态信息与交互。正式文件直读引擎长短屏/输入及10项回归通过；最终Creator导入待完成，未真实交易/构建/提交。见[交接](handoffs/2026-09-20-give-pad-approved.md)。
...
- 09-20：玩家列表「注册 / 登录」拆为「注册时间（北京时间）」+「最近登录（北京时间）」两列（`.player-table` 1120→1220px，时间口径未变），lint/tsc/build 通过并部署正式 8891，线上回读新表头命中、旧表头 0 命中，提交/推送 `7865427`、`ba38c31`（回滚点 `web.previous-player-time-split-20260920`）。**未登录页面视觉复核**，见[交接](handoffs/2026-09-20-xuanmanager-player-time-split.md)。
