...
for(const [role,level,prop,leader,total] of [['玩家','1','False',false,false],['盟主','99','False',true,false],['代理','98','True',false,false],['老板','99','True',true,false]]){Object.assign(account,{role,level,client_prop:prop});c.set_role();c.set_client_prop();assert.equal(node('操作').width,leader||prop==='True'?640:422);assert.equal(node('操作/我的盟主').active,leader);assert.equal(node('操作/总业绩').active,total);cases++;}
