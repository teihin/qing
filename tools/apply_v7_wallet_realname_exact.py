...
def apply_realname() -> None:
    ...
    body_h = (1672 - 103) * (750 / 941)
    p.art(background, "wallet_realname_body_v8_exact.png", 0, 0, 750, body_h)
    untint(p, background)
    root_top(p, background, 103 * (750 / 941), 750, body_h)

    title = p.node("钱包/实名/Title")
    p.art(title, "wallet_realname_header_exact.png", 0, 0, 750, 103 * (750 / 941))
    untint(p, title)
    root_top(p, title, 0, 750, 103 * (750 / 941), stretch_x=True)
